#!/usr/bin/env python3
"""Audit and evaluate the exact MIT lonespear CARE-production whole agent."""
...
try:
    from scripts.measure_leak_free_cv import canonical_sha256
except ModuleNotFoundError:
    from measure_leak_free_cv import canonical_sha256

ROOT = Path(__file__).resolve().parents[1]
CANDIDATE = ROOT / "candidates/lonespear-care-production/agent.py"
SOURCE = ROOT / "candidates/lonespear-care-production/source.json"
FIXTURE = ROOT / "tests/fixtures/lonespear_care_production.json"
OUTPUT = ROOT / "docs/measurements/SOT-2957/SOT-2959-lonespear-care-production.json"
...
def validate(fixture: dict, source: dict) -> dict[str, bool]:
    ...
    return {
        ...
        "vendored_license_hash": sha256(ROOT / "candidates/lonespear-care-production/LICENSE-MIT.txt") == source.get("license_sha256"),
        "stdlib_offline_with_optional_acceleration": imports <= {"numpy", "scipy"},
        "default_off_independent": source.get("default_enabled") is False and sha256(CHAMPION) != source.get("source_sha256"),
...
def run(policy: Path, panel: list[dict]) -> list[dict]:
    from kaggle_environments import make
    rows = []
    for identity in panel:
        lineup = [str(policy), str(ROOT / identity["opponent"])]
        if identity["seat"] == 1:
            lineup.reverse()
...
def main() -> int:
    ...
    report = {"issue":"SOT-2959", "axis":"lonespear coherent CARE-production independent whole-agent",
    ...
    if fixture["engine"] != f"kaggle-environments=={report['actual_engine']}" or not all(checks.values()):
        ...
        report["screen"] = {
            "champion_rows": run(CHAMPION, fixture["screen"]),
        ...
        if report["screen"]["gate"]["passed"]:
            report["confirm"] = {
                "champion_rows": run(CHAMPION, fixture["confirm"]),
        ...
        contract = all(r["statuses"] == ["DONE","DONE"] and r["steps"] == 720 for r in evaluated_rows)
        ...
        report["coherent_intervention"] = {
            "policy_components": ["early-capital", "CARE-livestock", "strawberry-fertilizer-production", "relative-margin-selling"],
            "exact_whole_agent": True,
            "individual_rejected_axes_retried": False,
        }
        report["decision"] = "promoted-independent-hedge" if promoted else "rejected-candidate-runtime-reverted"
