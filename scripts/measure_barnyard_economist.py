#!/usr/bin/env python3
"""Audit and evaluate the exact Apache-2.0 Barnyard Economist v5 hedge."""
...
CANDIDATE = ROOT / "candidates/barnyard-economist-v5/agent.py"
SOURCE = ROOT / "candidates/barnyard-economist-v5/source.json"
FIXTURE = ROOT / "tests/fixtures/barnyard_economist.json"
OUTPUT = ROOT / "docs/measurements/SOT-2957/SOT-2958-barnyard-economist.json"
...
def validate(fixture: dict, source: dict) -> dict[str, bool]:
    ...
    return {
        ...
        "source_version_hash_license_attribution": all(source.get(key) for key in (
            "source_url", "resolved_url", "kernel_version", "script_version_id",
            "source_path", "source_sha256", "license", "license_sha256")),
        "exact_source_hash": sha256(CANDIDATE) == source.get("source_sha256"),
        "vendored_license_hash": sha256(ROOT / "candidates/barnyard-economist-v5/LICENSE-Apache-2.0.txt") == source.get("license_sha256"),
...
def main() -> int:
    ...
    report = {"issue":"SOT-2958", "axis":"Barnyard Economist v5 independent whole-agent",
