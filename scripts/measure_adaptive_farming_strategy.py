"""Build and evaluate SOT-2977's clean-room adaptive whole-agent."""
...
PACKAGE = ROOT / "candidates/adaptive-farming-strategy"
...
OUTPUT = ROOT / "docs/measurements/SOT-2976/SOT-2977-adaptive-farming-strategy.json"
ENGINE = "1.32.7"
PANELS = {
    "screen": [
        ("barnyard-v5", ROOT / "candidates/barnyard-economist-v5/agent.py", 297701, 1),
        ("deepeshumrao", ROOT / "candidates/deepeshumrao-whole-agent/agent.py", 297703, 3),
    ],
    "confirm": [
        ("moon-v102", ROOT / "candidates/moon-counts-melons/agent.py", 297711, 11),
        ("soil-v26h", ROOT / "candidates/soil-remembers-rain/agent.py", 297713, 13),
    ],
}
...
def contract(path: Path) -> dict:
    spec = importlib.util.spec_from_file_location("adaptive_candidate_contract", path)
    ...
    routes = {
        "yarn_first": ["YARN_STORE"], "yarn_second": ["BAKERY", "YARN_STORE"],
        "yarn_third": ["BAKERY", "MARKET", "YARN_STORE"],
        "milk_support": ["PIZZA_SHOP"], "generalist": ["BAKERY"],
    }
    assert {name: module.select_route({"town": {"unlocked_shops": shops}}) for name, shops in routes.items()} == {name: name for name in routes}
    return {"entrypoint": True, "route_selector": True, "stdlib_only": True}
...
def run(policy: str, agent_path: Path, opponent_name: str, opponent: Path, seed: int, time_index: int, seat: int, cohort: str) -> dict:
    ...
    return {"cohort": cohort, "policy": policy, "opponent": opponent_name,
            "episode": f"adaptive-{cohort}-{opponent_name}", "lineage": opponent_name,
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="sot2977-") as directory:
        candidate = Path(directory) / "main.py"
        build_candidate(candidate)
        result = {"issue": "SOT-2977", "axis": "Adaptive Farming Strategy clean-room whole-agent",
