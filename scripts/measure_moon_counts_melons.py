"""Reproduce SOT-2972's preregistered independent whole-agent panel."""
...
OUTPUT = ROOT / "docs/measurements/SOT-2971/SOT-2972-moon-counts-melons.json"
ENGINE = "1.32.7"
PANELS = {
    "screen": [
        ("barnyard-v5", ROOT / "candidates/barnyard-economist-v5/agent.py", 297201, 1),
        ("deepeshumrao", ROOT / "candidates/deepeshumrao-whole-agent/agent.py", 297203, 3),
    ],
    "confirm": [
        ("lonespear-care", ROOT / "candidates/lonespear-care-production/agent.py", 297211, 11),
        ("opponent-shape", ROOT / "candidates/opponent-shape-portfolio/agent.py", 297213, 13),
    ],
}
...
def run(policy: str, agent: Path, opponent_name: str, opponent: Path, seed: int, time_index: int, seat: int, cohort: str) -> dict:
    ...
    return {
        ...
        "episode": f"moon-{cohort}-{opponent_name}", "seed": seed, "seat": seat,
...
def main() -> None:
    ...
    result = {"axis": "Moon Counts Melons V102 independent whole-agent", "actual_engine": ENGINE,
